...
#define Size(T)                               \
  int size_##T(vector_##T *vector) {          \
    return vector->ptr - (T *) vector->start; \
  }
#define Max_size(T)                                 \
  int max_size_##T(vector_##T *vector) {            \
    return (T *) vector->end - (T *) vector->start; \
  }
#define Isempty(T)                                \
  int isempty_##T(vector_##T *vector) {           \
    return (void *) vector->ptr == vector->start; \
  }
#define Push_back(T)                                      \
  void push_back_##T(vector_##T *vector, T val) {         \
...
#define Pop_back(T)                           \
  void pop_back_##T(vector_##T *vector) {     \
...
#define Remove(T)                                                                \
  void remove_##T(vector_##T *vec, int begin, int end) {                         \
...
#define Find(T)                           \
  int find_##T(vector_##T *vec, T val) {  \
...
#define Copyfrom(T)                                                               \
  void copyfrom_##T(vector_##T *dst, vector_##T *src, int srcbegin, int srcend) { \
...
#define Merge(T)                                                                                                                                 \
  void merge_##T(struct vector_##T *dst, struct vector_##T *src, int dstbegin, int srcbegin, int srcend) {                                       \
...
#define Clear(T)                    \
  void clear_##T(vector_##T *vec) { \
    vec->ptr = vec->start;          \
  }
#define NewVector(T)                                 \
  vector_##T *newvector_##T() {                      \
...
#define DeleteVector(T)                       \
  void deletevector_##T(vector_##T *vector) { \
    free(vector->start);                      \
    free(vector);                             \
  }
