...
#define Isfull(T)                             \
  int isfull_##T(stack_##T *stack) {          \
    return (void *) stack->ptr == stack->end; \
  }
#define Isempty(T)                              \
  int isempty_##T(stack_##T *stack) {           \
    return (void *) stack->ptr == stack->start; \
  }
#define Size(T)                             \
  int size_##T(stack_##T *stack) {          \
    return stack->ptr - (T *) stack->start; \
  }
#define Maxsize(T)                                \
  int maxsize_##T(stack_##T *stack) {             \
    return (T *) stack->end - (T *) stack->start; \
  }
#define NewStack(T)                                             \
  stack_##T *newStack_##T(int size) {                           \
...
#define DeleteStack(T)                     \
  void deleteStack_##T(stack_##T *stack) { \
    stack->ptr = NULL;                     \
    stack->end = NULL;                     \
    free(stack->start);                    \
    free(stack);                           \
