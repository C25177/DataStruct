...
#define DefaultCompare(T)                 \
  int compare_##T(const T a, const T b) { \
    if (a < b) return 1;                  \
    if (a == b) return 0;                 \
    return -1;                            \
  }
...
#define PriorityQueue(T)                                                                                      \
  typedef struct priority_queue_##T {                                                                         \
    void *start;                                                                                              \
    void *end;                                                                                                \
    T *ptr;                                                                                                   \
    ...
    T(*top)                                                                                                   \
    (struct priority_queue_##T * queue);                                                                      \
    T(*getval)                                                                                                \
    (struct priority_queue_##T * queue, int index);                                                           \
...
#define Push(T)                                                                                                       \
  void push_##T(priority_queue_##T *queue, const T val) {                                                             \
    ...
    while (right > left) {                                                                                            \
      ...
      else if (queue->compare(*it, val) < 0)                                                                          \
        left = mid + 1;                                                                                               \
      else {                                                                                                          \
        left = mid;                                                                                                   \
        break;                                                                                                        \
...
#define Pop(T)                                                                       \
  void pop_##T(priority_queue_##T *queue) {                                          \
...
#define Top(T)                                 \
  T top_##T(priority_queue_##T *queue) {       \
    if (queue->start == (void *) queue->ptr) { \
      printf("this queue has been empty\n");   \
      return (T) 0;                            \
    }                                          \
...
#define Size(T)                             \
  int size_##T(priority_queue_##T *queue) { \
    return queue->ptr - (T *) queue->start; \
  }
#define Maxsize(T)                                \
  int maxsize_##T(priority_queue_##T *queue) {    \
    return (T *) queue->end - (T *) queue->start; \
  }
#define Isempty(T)                              \
  int isempty_##T(priority_queue_##T *queue) {  \
    return queue->start == (void *) queue->ptr; \
  }
#define Clear(T)                              \
  void clear_##T(priority_queue_##T *queue) { \
    queue->ptr = queue->start;                \
  }
#define Find(T)                                          \
  int find_##T(priority_queue_##T *queue, const T val) { \
    int size = queue->ptr - (T *) queue->start;          \
...
#define GetVal(T)                                                 \
  T getval_##T(priority_queue_##T *queue, int index) {            \
...
#define Merge(T)                                                                               \
  void merge_##T(priority_queue_##T *dst, priority_queue_##T *src, int srcbegin, int srcend) { \
...
#define NewPriorityQueue(T)                                                                \
  priority_queue_##T *newpriorityqueue_##T(int (*compare)(const T a, const T b)) {         \
...
#define DeletePriorityQueue(T)                              \
  void deletepriorityqueue_##T(priority_queue_##T *queue) { \
    free(queue->start);                                     \
    free(queue);                                            \
  }
