...
#define Insert(T)                                                        \
  void insert_##T(linkedlist_##T *list, int index, listnode_##T *node) { \
    listnode_##T *iterator = list->head;                                 \
    while (iterator->next != list->tail && index--) {                    \
      iterator = iterator->next;                                         \
    }                                                                    \
    if (index > 0)                                                       \
      printf("index has out of range, but will insert tail\n");          \
...
#define Remove(T)                                            \
  void remove_##T(linkedlist_##T *list, int index) {         \
    listnode_##T *iterator = list->head;                     \
    if (!list->length) {                                     \
...
#define Merge(T)                                                                                            \
  void merge_##T(linkedlist_##T *dst, struct linkedlist_##T *src, int dstbegin, int srcbegin, int srcend) { \
    listnode_##T *dstit = dst->head;                                                                        \
    listnode_##T *srcit = src->head;                                                                        \
    while (dstit->next != dst->tail && dstbegin-- > 0)                                                      \
      dstit = dstit->next;                                                                                  \
    if (dstbegin > 0)                                                                                       \
      printf("dstbegin has out of range, so merge begin at the tail\n");                                    \
...
#define Clear(T)                           \
  void clear_##T(linkedlist_##T *list) {   \
...
#define Isempty(T)                         \
  int isempty_##T(linkedlist_##T *list) {  \
    return list->head->next == list->tail; \
  }
#define NewLinkedList(T)                                                      \
  linkedlist_##T *newLinkedList_##T() {                                       \
...
#define DeleteLinkedList(T)                       \
  void deleteLinkList_##T(linkedlist_##T *list) { \
    listnode_##T *iterator = list->head;          \
    while (iterator) {                            \
