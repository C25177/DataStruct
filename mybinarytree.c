...
#define Queue(T)                                                       \
  typedef struct queuetreenode_##T {                                   \
    void *start;                                                       \
    void *end;                                                         \
...
#define Push(T)                                                               \
  void pushtreenode_##T(queuetreenode_##T *queue, struct treenode_##T *val) { \
...
#define Pop(T)                                                                                                        \
  void poptreenode_##T(queuetreenode_##T *queue) {                                                                    \
...
#define Top(T)                                                  \
  treenode_##T *toptreenode_##T(queuetreenode_##T *queue) {     \
    treenode_##T *treenode = *((treenode_##T **) queue->start); \
    return treenode;                                            \
  }
#define NewQueue(T)                                                                     \
  queuetreenode_##T *newqueuetreenode_##T(int size) {                                   \
...
#define DeleteQueue(T)                                     \
  void deletequeuetreenode_##T(queuetreenode_##T *queue) { \
    free(queue->start);                                    \
    free(queue);                                           \
  }
...
#define NodeDFS(T)                                            \
  void nodeDFS_##T(treenode_##T *this, void (*func)(T val)) { \
    if (!this) return;                                        \
    func(this->val);                                          \
...
#define NodeBFS(T)                                            \
  void nodeBFS_##T(treenode_##T *this, void (*func)(T val)) { \
...
#define InsertL(T)                                                                      \
  void insertl_##T(treenode_##T *this, treenode_##T *node) {                            \
    this->left = node;                                                                  \
    this->height = (1 + node->height > this->height ? 1 + node->height : this->height); \
    node->parent = this;                                                                \
  }
#define InsertR(T)                                                                      \
  void insertr_##T(treenode_##T *this, treenode_##T *node) {                            \
    this->right = node;                                                                 \
    this->height = (1 + node->height > this->height ? 1 + node->height : this->height); \
    node->parent = this;                                                                \
  }
#define Isleaf(T)                       \
  int isleaf_##T(treenode_##T *this) {  \
    return !this->right && !this->left; \
  }
#define Isroot(T)                      \
  int isroot_##T(treenode_##T *this) { \
    return !this->parent;              \
  }
#define NewTreeNode(T)                                                  \
  treenode_##T *newtreenode_##T(T val) {                                \
...
#define DeleteTreeNode(T)                       \
  void deletetreenode_##T(treenode_##T *node) { \
    if (!node) return;                          \
...
#define Isempty(T)                  \
  int isempty_##T(tree_##T *this) { \
    return this->size == 0;         \
  }
#define Getroot(T)                            \
  treenode_##T *getroot_##T(tree_##T *this) { \
    return this->root;                        \
  }
#define HeightBFS(T)                                 \
  int heightBFS_##T(treenode_##T *node) {            \
...
#define Updateheight(T)                   \
  void updateheight_##T(tree_##T *this) { \
    this->heightBFS(this->root);          \
  }
#define Insert(T)                                              \
  void insert_##T(tree_##T *this, T val) {                     \
...
#define BalanceBFS(T)                                          \
  void balanceBFS_##T(queue(T) * queue, treenode_##T * node) { \
    if (!node) return;                                         \
...
#define LinkBFS(T)                                                                        \
  void linkBFS_##T(queue(T) * queue, treenode_##T * node, int left, int right, int mid) { \
...
#define DeleteTree(T)                   \
  void deletetree_##T(tree_##T *tree) { \
...
int main() {
  ...
  tree->insert(tree, -1);
  tree->insert(tree, -2);
  tree->root->nodeDFS(tree->root, 1);
  tree->root->nodeBFS(tree->root, 2);
  deletetree(int)(tree);
}
